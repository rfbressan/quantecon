...
class Solow:
    ...
    def __init__(
        self, savings: float, delta: float, k_0: float, production: Production
    ):
    ...
    def simulate_ts(self, k_init, ts_length):
        """Simulate the time series of capital per capita given an initial capital stock

        :param k_init: initial capital per capita
        :type k_init: float
        :param ts_length: length of the time series
        :type ts_length: int
        :return: time series of capital per capita
        :rtype: float numpy array
        """
